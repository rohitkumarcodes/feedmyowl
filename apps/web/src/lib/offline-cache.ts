...
/**
 * Persist the latest feed/folder state for offline fallback use.
 */
export async function saveWorkspaceSnapshot(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    request.onerror = () =>
      reject(request.error ?? new Error("Snapshot save failed"));
  });
...
export async function loadWorkspaceSnapshot(): Promise<WorkspaceSnapshot | null> {
  ...
  const snapshot = await new Promise<WorkspaceSnapshot | null>(
    (resolve, reject) => {
      ...
      request.onerror = () =>
        reject(request.error ?? new Error("Snapshot load failed"));
    }
  );
