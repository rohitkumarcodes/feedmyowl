...
describe("retention helpers", () => {
  ...
  it("returns 0 when no rows exceed the per-feed cap", async () => {
    mocks.dbExecute.mockResolvedValue({ rows: [] });

    const deletedCount = await purgeOldFeedItems();
  ...
  it("returns deleted row count when prune removes over-limit items", async () => {
  ...
  it("feed-scoped prune safely returns 0 for missing feeds", async () => {
    mocks.dbExecute.mockResolvedValue({ rows: [] });
  ...
  it("returns false when no feed exceeds the retention cap", async () => {
    ...
    const shouldPurge = await isRetentionPurgeNeeded();
