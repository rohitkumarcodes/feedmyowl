...
Follow-up (2026-02-11): the feed pipeline now runs with hardened reliability/security defaults: feed fetch hardening (SSRF blocking, redirect revalidation, timeout + retries), conditional fetch support (ETag / Last-Modified), and reliable dedupe with DB-backed GUID/fingerprint uniqueness.
...
Follow-up (2026-02-12): feed discovery for site URLs now includes `www.<host>` fallback probing and explicit submit-failure messaging so add-feed does not appear unresponsive on transient errors.
